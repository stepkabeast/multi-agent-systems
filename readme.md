...
### Задача 1ж (ветка practice_07_ex_1zh)
Откатываем ContactNet (опционально).
Делаем так, что вычислители запускаются без координатора и назначают его по одному из алгоритмов выбора (на ваше усмотрение). Назначенный координатор регистрируется в DF, чтобы заказчик его там нашёл.  
Логику выборов точно нужно делать отдельным поведением.

(задача со звёздочкой) Вернуть ContactNet, если откатили.

![without_coord_start.png](./img/ex_07_1zh/without_coord_start.png)